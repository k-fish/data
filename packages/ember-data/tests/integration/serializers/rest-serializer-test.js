...
test("modelNameFromPayloadKey returns always same typeKey even for uncountable multi words keys", function() {
  expect(2);
  Ember.Inflector.inflector.uncountable('words');
  var expectedTypeKey = 'multiWords';
  equal(env.restSerializer.modelNameFromPayloadKey('multi_words'), expectedTypeKey);
  equal(env.restSerializer.modelNameFromPayloadKey('multiWords'), expectedTypeKey);
});
